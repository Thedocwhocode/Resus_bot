...
def test_classify_resuscitation():
    data = {"title": "Post-cardiac arrest targeted temperature management", "journal": "Resuscitation"}
    assert classify_article(data) == 1  # resuscitation


def test_classify_sepsis():
    data = {"title": "ROX index to predict failure of high-flow nasal cannula in septic patients", "journal": "AJRCCM"}
    assert classify_article(data) == 2  # sepsis


def test_classify_airway():
    data = {"title": "Video laryngoscopy versus direct laryngoscopy for orotracheal intubation", "journal": "NEJM"}
